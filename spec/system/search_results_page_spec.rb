...
describe 'Search Results Page', :js do
  before do
  ...
  it 'renders expected fields' do
    ...
    expect(page).to have_text 'Stotler, D.; Scotti, F.; Bell, R. E.; Diallo, A.' # authors
    expect(page).to have_text 'Version: ' # displays version number
  ...
  it 'links to the expected facet values' do
    ...
    expect(find_link('NSTX')[:href]).to include('collection_tag_ssim')
  end
  ...
  describe 'author searches' do
    ...
    it 'finds record by author synonym' do
      ...
      expect(page).to have_text("Author(s):\nTrueman, Daniel")
  ...
  describe 'bookmarks' do
    it 'does not render bookmark checkboxes' do
      visit '/?search_field=author&q=podesta'
      expect(page).to have_no_css 'div.toggle-bookmark'
    end
  end
