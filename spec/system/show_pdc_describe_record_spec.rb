...
describe "Show PDC Page", :js do
  before do
    ...
    pdc_files.each do |name|
      stub_request(:get, "https://datacommons.princeton.edu/describe/works/#{name}")
        .to_return(status: 200, body: Rails.root.join("spec/fixtures/files/pdc_describe_data/#{name}").read, headers: {})
    end
    stub_request(:get, "http://pdc_test_data/works.rss")
      .to_return(status: 200, body: Rails.root.join("spec/fixtures/files/pdc_describe_data/works.rss").read, headers: {})
  ...
  it "renders total file size" do
    ...
    expect(page).to have_text("Total Size")
    expect(page).to have_text("391 MB")
  end

  it "renders rights statement" do
    visit "/catalog/doi-10-34770-bm4s-t361"
    expect(page).to have_text("Creative Commons Attribution 4.0 International (CC BY)")
  ...
  it "shows file types when there is more than 10 in sidebar" do
    ...
    expect(page).to have_css("#document-file-type-list-toggle", visible: true)
    ...
    expect(page).to have_css("#document-file-type-list-extra", visible: true)
  ...
  context "when crawler visits the site" do
    ...
    it "does not display links when there is a crawler" do
      visit "/catalog/doi-10-34770-bm4s-t361"
      expect(page).to have_no_link(href: "https://g-ef94ef.f0ad1.36fe.data.globus.org/10.34770/bm4s-t361/89/Fig11b_readme.hdf")
    end
  end
  ...
  context "clicks on Find other works by this author" do
    it "uses link with lowercase orcid" do
      ...
      expect(page).to have_text("Find other works by this author in PDC Discovery.")
