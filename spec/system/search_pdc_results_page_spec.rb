...
describe 'Search Results PDC Page', :js do
  before do
  ...
  it 'renders expected fields' do
    ...
    expect(page).to have_text('Subcommunity')
    click_on('Subcommunity')
    expect(page).to have_text('Stellarators')
    expect(page).to have_text('more')
    click_link('more')
    expect(page).to have_text('System Studies')
    click_on('System Studies')
    expect(page).to have_text('Fusion Pilot Plant performance and the role of a Sustained High Power Density tokamak')

    click_on 'Year Published'
    expect(page).to have_text 'View larger'
