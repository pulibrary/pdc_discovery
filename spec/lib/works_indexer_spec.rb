...
RSpec.describe WorksIndexer do
  describe 'indexing a single record' do
    ...
    context 'basic fields' do
      ...
      it 'contributors' do
        expect(indexed_record['contributor_tsim'].include?('Villalta, Andrés')).to be true
        expect(indexed_record['contributor_tsim'].include?('Chou, Katie')).to be true
        expect(indexed_record['contributor_tsim'].include?('Ayres, Christien')).to be true
        expect(indexed_record['contributor_tsim'].include?('Wang, Matthew')).to be true
      end
      ...
      it 'title' do
        ...
        expect(indexed_record['title_tesim'].include?(main_title)).to be true
        expect(indexed_record['title_tesim'].include?(alt_title)).to be true
        # alt title does not include the main title
        expect(indexed_record['alternative_title_tesim'].include?(main_title)).to be false
        expect(indexed_record['alternative_title_tesim'].include?(alt_title)).to be true
      end
      ...
      it 'keywords' do
        expect(indexed_record['subject_all_ssim'].include?('keyword1')).to be true
        expect(indexed_record['subject_all_ssim'].include?('keyword2')).to be true
        expect(indexed_record['subject_all_ssim'].include?('keyword3')).to be true
      end

      it 'collection tag' do
        expect(indexed_record['collection_tag_ssim'].include?('Humanities')).to be true
        expect(indexed_record['collection_tag_ssim'].include?('Something else')).to be true
      end
      ...
      it 'migrated_bsi' do
        expect(indexed_record['migrated_bsi']).to be true
      end
    ...
    context 'uris' do
      it 'stores full URL for ARK and DOI' do
        expect(indexed_record['uri_ssim'].include?('http://arks.princeton.edu/ark:/88435/dsp015999n653h')).to be true
        expect(indexed_record['uri_ssim'].include?('https://doi.org/10.34770/r75s-9j74')).to be true
      end
    end
