...
##
# Fetch an RSS feed of draft works (a.k.a. "awaiting approval", "draft DOIs") from PDC Describe. For each one, 
# index the DOI to solr with a "draft" status, so that the DOI resolves to a landing page in the PDC Discovery UI 
# even before the work is published.
class DraftWorksIndexer
  ##
  # See config/pdc_discovery.yml for configuration of the RSS feed that
  # this indexer uses to harvest data from PDC Describe.
  # TODO: This now needs to be refactored to allow for three different feeds.
